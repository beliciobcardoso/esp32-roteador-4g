...
**Estado atual:** Fases 1-6 implementadas e validadas em hardware — storage NVS, SoftAP, config HTTP, modem PPP, NAT/roteamento e supervisão do uplink com reconexão automática. Um celular conectado no AP navega pelo 4G, e o enlace se recupera sozinho de queda de RF (~16 s) e de perda do SIM (backoff até reboot). As Fases 7 (relógio por SNTP e fuso configurável) e 8 (atualização de firmware pela própria página, com rollback do bootloader) foram **validadas em placa em 20/09/2026**: relógio certo 17 s depois do reset, fuso trocando a quente, OTA trocando de slot e rollback revertendo de verdade um reset dentro da janela. Segue aberto um único critério de bancada — upload interrompido no meio. Da Fase 9 (telemetria MQTT, [PRD 14](docs/prd/14-telemetria-mqtt.md)) existem **o domínio e o contador acumulado**: `domain/telemetry`, `domain/telemetry_buffer` e `domain/mqtt_backoff`, testados no host em 23/09/2026, e o total de descartes do PPP desde o boot em `infra/ppp_drop_counter`, que já aparece em `/api/status` e na página (validado em placa em 24/09/2026). Nada ainda publica por MQTT: falta `infra/mqtt_client` e tudo o que toca a rede. A Fase 10 (acesso remoto à página, [PRD 13](docs/prd/13-acesso-remoto.md)) segue só proposta. Da Fase 11 (posição por GNSS, [PRD 15](docs/prd/15-gps-posicao.md)) existe só a identificação do modem: o `AT+SIMCOMATI` de todo boot respondeu `A7670E-FASE` em 24/09/2026, a variante com GNSS interno, e isso desbloqueou a fase — CMUX e leitura de GNSS não começaram. PRDs em [docs/prd/](docs/prd/), ressalvas por fase em [docs/PLANO_ROTEADOR.md](docs/PLANO_ROTEADOR.md).
